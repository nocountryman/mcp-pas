...
@mcp.resource("pas://antigravity")
def get_antigravity_context() -> str:
    """Returns agent trajectories and user memories from Antigravity.
    
    Phase 32: Provides access to:
    - Recent agent session trajectories (last 10)
    - User memories (agent rules and guidelines)
    - MCP server states
    
    Requires active Antigravity IDE instance.
    """
    try:
        from pas.helpers.antigravity_client import get_antigravity_client
        client = get_antigravity_client()
        
        if not client.primary:
            return json.dumps({
                "error": "No Antigravity instance found",
                "hint": "Ensure Antigravity IDE is running"
            })
        
        # Get recent trajectories (summaries only)
        trajectories = client.get_all_trajectories()
        trajectory_dict = trajectories.get("trajectorySummaries", {})
        # API returns dict keyed by cascade ID - convert to list sorted by most recent
        trajectory_list = list(trajectory_dict.items())[:10]
        ...
        return json.dumps({
            "instance": {
                "port": client.primary.port,
                "workspace": client.primary.workspace_id
            },
            "recent_trajectories": [
                {
                    "id": cascade_id,
                ...
                for cascade_id, data in trajectory_list
            ],
            "user_memories": [
                {
                    "id": m.get("id"),
                ...
                for m in (list(memory_list.values()) if isinstance(memory_list, dict) else memory_list)[:5]
            ],
            "trajectory_count": len(trajectory_dict),
...
async def record_outcome(
    ...
    try:
        ...
        cur.execute(
            "SELECT session_mode, state, goal FROM reasoning_sessions WHERE id = %s",
            (session_id,)
        )
        ...
        session_goal = session_row.get("goal", "")  # Phase 34: for trajectory matching
        mode_config = MODE_CONFIG.get(session_mode, MODE_CONFIG["implementation"])
        ...
        # Cross-Project Learning: Extract generalized pattern
        try:
            from pas.helpers.pattern_extraction import extract_generalized_pattern
            pattern_data = extract_generalized_pattern(failure_reason, notes)
            
            # Generate embedding for generalized pattern (for cross-project semantic search)
            generalized_embedding = None
            if pattern_data["generalized_pattern"]:
                try:
                    generalized_embedding = get_embedding(pattern_data["generalized_pattern"])
                except Exception:
                    pass
            
            cur.execute("""
                UPDATE outcome_records 
                SET generalized_pattern = %s, 
                    pattern_type = %s,
                    generalized_embedding = %s
                WHERE id = %s
            """, (
                pattern_data["generalized_pattern"],
                pattern_data["pattern_type"],
                generalized_embedding,
                record["outcome_id"]
            ))
            logger.debug(f"Cross-project pattern: {pattern_data['pattern_type']}")
        except Exception as e:
            logger.warning(f"Pattern extraction failed (non-fatal): {e}")
        ...
        # Phase 34: Capture trajectory pattern (sync with timeout)
        trajectory_id = None
        try:
            from pas.helpers.trajectory_learning import capture_current_trajectory
            try:
                from pas.helpers.external_services import get_embedding as embedding_fn
            except Exception:
                embedding_fn = None  # Embedding service may be unavailable
            trajectory_id = capture_current_trajectory(
                session_id=session_id,
                outcome=outcome,
                session_goal=session_goal,
                conn=conn,
                get_embedding_fn=embedding_fn
            )
        except Exception as e:
            logger.warning(f"Phase 34: Trajectory capture failed (non-fatal): {e}")
    ...
    try:
        ...
        for file_path in path.rglob('*'):
            ...
            try:
                ...
                symbols = []
                # v53: Enable LSP for Python and C#
                ...
                if (include_references or include_call_hierarchy) and language in ("python", "csharp"):
...
async def store_research(
    ...
    """
    DB-first research storage.
    
    Stores research to artifacts table first (ENFORCED),
    then optionally exports to file.
    
    Args:
        project_id: Project identifier
        topic: Research topic (becomes artifact name)
        content: Full research content (markdown)
        export_path: Optional file path for export (secondary copy)
        session_id: Link to PAS reasoning session
        tags: Comma-separated tags
    
    Returns:
        Artifact ID and export status
    """
    try:
        # 1. Store to DB first (ENFORCED)
        tag_list = [t.strip() for t in tags.split(",")] if tags else []
        if "research" not in tag_list:
        ...
        # 2. Optional file export (secondary copy)
        exported = None
        if export_path:
            from pathlib import Path
            ...
            exported = str(export_file)
            logger.info(f"Research exported to {exported}")
        ...
        return {
            ...
            "message": f"Research '{topic}' stored in DB. This is the source of truth.",
            "next_step": "File export is secondary. Use pas://artifacts/{project_id} to search."
        }
    except Exception as e:
        logger.error(f"store_research failed: {e}")
        return {"success": False, "error": str(e)}
...
# Phase 31: Multi-Model Verification
@mcp.tool()
async def verify_externally(
    node_id: str,
    context: Optional[str] = None
) -> dict[str, Any]:
    """
    Verify a hypothesis using external model (Gemini CLI).
    
    Phase 31: Adversarial verification to eliminate self-confirmation bias.
    Uses Gemini CLI subprocess to get independent critique from a different model.
    
    Args:
        node_id: The thought node UUID to verify
        context: Optional additional context (goal, constraints)
        
    Returns:
        External critique with major_flaws, minor_issues, and confidence
    """
    from pas.helpers.gemini_verifier import verify_with_gemini
    ...
    try:
        ...
        # RealDictRow - access by key, not tuple unpacking
        content = row['content']
        scope = row['declared_scope']
        hypothesis = f"{content}\n\nScope: {scope}" if scope else content
        
        # Call Gemini CLI for verification
        critique = await verify_with_gemini(hypothesis, context)
        
        # Store result in external_critique column
        cur.execute("""
            UPDATE thought_nodes 
            SET external_critique = %s::jsonb
            WHERE id = %s::uuid
        """, (json.dumps(critique), node_id))
        conn.commit()
